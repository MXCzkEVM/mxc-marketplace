...
contract MXCMarketPlaceUpgradeV6 is UUPSUpgradeable, IERC721Receiver {
    ...
    function executeOrderByMiner(address nftAddress, uint256 assetId) external payable {
      executeOrderByErc721(nftAddress, assetId, 25);
    }

    function executeOrderByHexagon(address nftAddress, uint256 assetId) external payable {
      executeOrderByErc721(nftAddress, assetId, 0);
    }

    function executeOrderByErc721(address nftAddress, uint256 assetId, uint256 feeBps) internal {
      _verifyOrder(nftAddress, assetId);

      Order memory order = orderByAssetId[nftAddress][assetId];
      IERC721 token = IERC721(nftAddress);

      uint256 fee =  feeBps != 0 ? (order.price * feeBps / 1000) : 0;
      uint256 price = order.price - fee;
