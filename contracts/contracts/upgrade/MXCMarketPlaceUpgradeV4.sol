...
contract MXCMarketplaceUpgradeV4 is UUPSUpgradeable, IERC721Receiver {
    ...
    function createOrder(
        ...
        address assetOwner = _getAssetOwner(nftAddress, assetId);
    ...
    function executeNameOrder(uint256 assetId) external payable {
        Order memory order = orderByAssetId[nameToken][assetId];
        ...
        IMXCName hexagonTokenCls = IMXCName(nameToken);
        if (order.seller != hexagonTokenCls.ownerOf(assetId)) {
            revert MXCMarketplace__SellerNotOwner();
        }
        ...
        // send nft to buyer
        hexagonTokenCls.safeTransferFrom(order.seller, msg.sender, assetId);

        delete orderByAssetId[nameToken][assetId];
        emit OrderSuccessful(
            order.id,
            assetId,
            order.seller,
    ...
    function executeDomainOrder(uint256 assetId) external payable {
        Order memory order = orderByAssetId[domainToken][assetId];
        ...
        IMXCDomain token = IMXCDomain(domainToken);
        if (order.seller != token.ownerOf(assetId)) {
            revert MXCMarketplace__SellerNotOwner();
        }
        ...
        delete orderByAssetId[domainToken][assetId];
